...
(() => {
  ...
  function installViewportGuard() {
    ...
    if (root.dataset.scViewportGuard === "1") return;
    root.dataset.scViewportGuard = "1";
    const style = doc.createElement("style");
    style.textContent = `
      :root{--app-height:100vh;--app-vv-top:0px;--app-keyboard-backdrop:#F0E9DC}
      @supports(height:100dvh){:root{--app-height:100dvh}}
      html,body{height:var(--app-height)!important;min-height:var(--app-height)!important;overflow:hidden!important;background:#16140f}
      body{width:100%;overscroll-behavior:none;-webkit-overflow-scrolling:auto}
      #dc-root,#dc-root>.sc-host{height:var(--app-height)!important;min-height:var(--app-height)!important;overflow:hidden!important}
      html.sc-keyboard-open{background:var(--app-keyboard-backdrop)!important}
      html.sc-keyboard-open body{position:fixed;inset:0;background:var(--app-keyboard-backdrop)!important}
      .sc-keyboard-backfill{position:fixed;left:0;right:0;top:calc(var(--app-height) - 1px);height:60vh;background:var(--app-keyboard-backdrop);opacity:0;pointer-events:none;z-index:0}
      html.sc-keyboard-open .sc-keyboard-backfill{opacity:1}
    `;
    doc.head.appendChild(style);
    let backfill = null;
    const ensureBackfill = () => {
      if (backfill || !doc.body) return;
      backfill = doc.createElement("div");
      backfill.className = "sc-keyboard-backfill";
      doc.body.appendChild(backfill);
    };
    let raf = 0;
    ...
    const sync = () => {
      ...
      root.style.setProperty("--app-vv-top", `${Math.max(0, Math.round(top))}px`);
      root.classList.toggle("sc-keyboard-open", !!vv && window.innerHeight - vv.height > 120);
